...
export default function BookingClient(){
 ...
 return <div className="booking-card request-card">
   ...
   <form onSubmit={submit}><div className="formgrid"><label>Your Name<input name="parentName" required/></label><label>Student Name <small>(optional)</small><input name="studentName"/></label><label>Email<input name="email" type="email" required/></label><label>Phone<input name="phone"/></label></div><label>Message / swimming goals <small>(optional)</small><textarea name="notes" rows={3}/></label><button className="btn booking-submit" disabled={sent}>{sent?'Request Sent ✓':'Send Request →'}</button><p className="success" aria-live="polite">{msg}</p></form>
 </div>
}

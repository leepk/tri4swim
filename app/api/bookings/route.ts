...
export async function POST(req:NextRequest){
 try{
  ...
  const booking=await prisma.booking.create({data:{lessonTypeId:lesson?.id||null,start,end,parentName:String(b.parentName||'').trim(),studentName:b.studentName||null,email:String(b.email||'').trim(),phone:b.phone||null,notes:b.notes||null,source:adminSource?'ADMIN':'WEBSITE',status:adminSource?'CONFIRMED':'PENDING'},include:{lessonType:true}})
  if(!adminSource) {
    await sendBookingEmail(booking.email,'Tri4Swim booking request received',`<p>Hi ${booking.parentName},</p><p>We received your preferred swim date and time: <strong>${start.toLocaleString()}</strong>.</p><p>This is a booking request. The coach will contact you to confirm the course details and schedule.</p>`).catch(console.error)

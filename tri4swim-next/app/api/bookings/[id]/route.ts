...
export async function PATCH(req:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(status==='CONFIRMED' && updated.email) await sendBookingEmail(updated.email,'Your Tri4Swim lesson is confirmed',`<h2>Your Tri4Swim time is confirmed</h2><p>${updated.lessonType?.name || 'Swim lesson'}</p><p>${updated.start.toLocaleString()}</p>`)
 if(status==='CANCELLED' && updated.email) await sendBookingEmail(updated.email,'Tri4Swim lesson cancelled',`<p>Your lesson on ${updated.start.toLocaleString()} has been cancelled.</p>`)

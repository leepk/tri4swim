...
export async function GET(_r:NextRequest,{params}:{params:Promise<{path:string[]}>}){
  ...
  try{const file=path.join(dir,parts[0]),data=await readFile(file),type=types[path.extname(file).toLowerCase()]||'application/octet-stream';return new NextResponse(data,{headers:{'content-type':type,'cache-control':'public, max-age=31536000, immutable'}})}catch{return new NextResponse('Not found',{status:404})}
}
